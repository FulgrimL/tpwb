...
function setPhoneCustomer(arr, index) {
    ...
    var sex = arr.sex,
        ...
        email = arr.email
    var idType = Number(idType)

    var isGroup = getUrlParam('prdId')
        // 驾乘险系列
    if (isGroup == "9" || isGroup == "13" || isGroup == "67" || isGroup == "68" || isGroup == "81" || isGroup == "86" || isGroup == "87" || isGroup == "186" || isGroup == "187" || isGroup == "219" || isGroup == "220" || isGroup == "221" || isGroup == "222" || isGroup == "223" || isGroup == "224" || isGroup == "225" || isGroup =="226") {
        console.log('1')
            // mui.alert("1")
        if (judgeType('#GRPNAME')) {
            ...
            switch (idType) {
                case 1:
                    $('#GRPIDTYPE').html('身份证').attr('value', 'I')
                    break
                case 2:
                    $('#GRPIDTYPE').html('护照').attr('value', 'I')
                    break
                case 3:
                    $('#GRPIDTYPE').html('军人证').attr('value', 'I')
                    break
            }
            console.log('1')
            $('#GRPIDNO').val(idNo).trigger('keyup')
            console.log('1')
...
function createJob($parentCotroller, textJson, isSafe) {
                    ...
                    $('#jobLevel2 li').click(function() {
                        ...
                        GetData('profession/selectCode', function(res) {
                            if (isSafe == 1) {
                                ...
                                console.log("$('#jobLevel3 ul li').length=" + $('#jobLevel3 ul li').length)
                                setTimeout(function() {
                                    if ($('#jobLevel3 ul li').length == 0) {
                                        var jobkey = $('#jobSelect').attr('data-keyName')

                                        mui.alert("该职业类别暂时无法投保！");
                                        // 清空重置初始状态
                                        $(".jobpage ul").html("")
                                        $("#jobLevel1").addClass("cur").siblings(".jobpage").removeClass("cur")
...
// 根据所给json创建页面
function createPage(productJson, isSafe) {
    $.each(productJson, function(k, v) {
        switch (v.modelType) {
            case 'home':
                ...
                $.each(v.modelColumn, function(k, v) {
                    ...
                    if (v.type == 'label') {
                        ...
                        titleHtml = '<div class="mui-input-row row-title expand">' +
                            ...
                            '</div>'
                            // edit_ocr 2018/5/11
                            /*var xxx_html1 = '<div class="mui-input-row row-title expand">' +
                                '<div class="row-title-text">' + v.groupName + '</div>' +
                                '<img src="../images/scan.png" class="scanButton" alt="' + v.event + '">' +
                                '</div>'
                            var xxx_html2 = '<div class="mui-input-row row-title expand">' +
                                '<div class="row-title-text">' + v.groupName + '</div>' +
                                '<img src="../images/scan.png" class="scanButton" alt="' + v.event + '">' +
                                '<input style="opacity: 0;width: 100%;height: 40px;position: absolute;" id="input" type="file" accept="image*/
                            /*" capture="camera" onchange="getImage(this)">'+
                                                        '</div>'
                                                    var xxx = getUrlParam("prdId")
                                                    console.warn(xxx)
                                                    xxx == "239"? titleHtml = xxx_html2 : titleHtml = xxx_html1*/
                    }
                            ...
                            $.ajax({
                                ...
                                success: function(data) {
                                    ...
                                    if (data.code == '0') {
                                        ...
                                        var sharePictureUrl = httpLocation + data.data.sharePictureUrl
                    ...
                    $.each(cpxxJson, function(k, v) {
                        switch (v.type) {
                            case 'text':
                                ...
                                break
                            case 'job':
                                createJob($('#page1 .pt-triggers .section').eq(homeNum), v, isSafe)
                                break
                            case 'phoneNumber':
                    ...
                    $.each(groupColumn, function(k, v) {
                        switch (v.type) {
                            case 'text':
                                ...
                                break
                            case 'job':
                                createJob($thisPage.find('.section'), v, isSafe)
                                break
                            case 'phoneNumber':
            ...
            case 'listPage4':
                ...
                if ($('.secList').length == 0) {
                    secListHtml = '<div class="mui-input-group section secList">' +
                        ...
                        '<a class="mui-navigate-right" id="' + v.xmlNode + '">' +
                        v.modelName +
                        '<span class="listPageTip" id="listPageTip4"></span></a>' +
                        '</li></ul></div>'
                    $('#page1 .buttonBlock').before(secListHtml)
                } else {
                    $('#page1 .secList ul').append('<li class="mui-table-view-cell"><a class="mui-navigate-right" id="' + v.xmlNode + '">' + v.modelName + '<span class="listPageTip" id="listPageTip4"></span></a></li>')
                }

                // 生成listPage1页面
                $('#pt-main').append('<div class="pt-page" id="listPage4" last-page="page1"><div class="pt-triggers"><div class="row-group section info"></div></div></div>')
                    // 生成cardPage
                var cardPageHtml = '<div class="pt-page" id="cardPage4" last-page="page1">' +
                    '<div class="pt-triggers">'

                +'<div class="buttonBlock">' +
                '<button type="button" class="mui-btn mui-btn-block card-button" id="card4AddBtn">添加</button>' +
                '<button type="button" class="mui-btn mui-btn-block card-button" id="card4SaveBtn">保存</button>' +
                '</div></div></div>'
                $('#pt-main').append(cardPageHtml)
                    // 被保人数
                var xtdPerNum = '<div class="mui-input-row"><label>被保人数</label><input type="text" placeholder="" id="xtdPerNum" readonly="readonly" value=""></div>'
                $('.secList .mui-table-view').append(xtdPerNum)
                    // 被保人信息
                var xtdPerInf = '<div class="mui-input-row"><label>被保人信息</label><label id=xtdPerVal></label><button id=xtdBtn>录入信息</button></div>'
                $('.secList .mui-table-view').append(xtdPerInf)
                    // 生成模拟列表
                    // var cardPageHtml2 = '<li class="mui-card"><ul style="padding-left:0"><li><a id=icon-close class=liIcon><span class="mui-icon mui-icon-close"></span></a><span class=liNum>1</span><div class=comtent><p><span>姓名</span>:&nbsp;<span>张三</span></p><p><span>身份证</span>:&nbsp;<span>330382198607226940</span></p></div></ul></li>'
                    // $(".secList .mui-table-view").append(cardPageHtml2)

                // 生成提交按钮
                if ($('#listPage4Submit').length == 0) {
                    var btnHtml = '<div class="buttonBlock">' +
                        '<button type="button" class="mui-btn mui-btn-block card-button" id="listPage4Submit">保存</button>' +
                        '</div>'
                    $('#listPage4 .pt-triggers').append(btnHtml)
                }
                var $thisPage = $('#listPage4')
                var modelColumn = v.modelColumn
                $.each(modelColumn, function(k, v) {
                    if (v.type == 'label') {
                        var headHtml = '<div class="title-row">' +
                ...
                $('#listPage4Submit').click(function() {
                    // listPage4flag = true;//重置标志位
                    var liNum = $('.linum').length
                    console.log('length=' + liNum)
                        // 1.先判断是添加还是修改状态
                    if (cardStatus4 == 'add') {
                        // 跳转页面之前先校验
                        console.log(validNull('listPage4'))
                        console.log(validPage('listPage4'))
                        if (validNull('listPage4') && validPage('listPage4')) {
                            console.log('change')
                            changePage('listPage4', 'page1', 'last')
                        }
                        if (validPage('listPage4')) {
                            var listPageObj = {}
                            $.each(modelColumn, function(k, v) {
                                var groupColumn = v.groupColumn
                                $.each(groupColumn, function(k, v) {
                            ...
                            $('#listPage4').append("<input type='hidden' value='" + JSON.stringify(listPageObj) + "' class='listPage4HiddenInput'>")
                                // 小团单 挪至首页
                                // 生成card页面基本信息
                            var cardHtml = '<div class="row-group section cardinfo4"><div class="card-row4">' +
                                '<img src="../images/product/card-icon.png" class="card4-icon">' +
                                '<label class="cardName4">' + listPageObj.ISDNAME + '</label>' +
                                '<img src="../images/product/delete.png" class="deleteCardBtn" style="top:15px;right:10px;">' +
                                '</div>' +
                                '<div class="card-row" style="font-size:15px;padding-left:45px;">' +
                                '证件号：<label class="cardIdno4">' + listPageObj.ISDIDNO + '</label>' +
                                '</div></div>'

                            $('#cardPage4 .buttonBlock').before(cardHtml)

                            // 生成首页列表
                            var cardPageHtml2 = '<li class="mui-card linum section cardinfo4"><ul style="padding-left:0"><li><a id=icon-close class=liIcon><span class="mui-icon mui-icon-close deleteCardBtn"></span></a><span class=liNum>' + (liNum + 1) + '</span><div class=comtent><p><span class=cardName4>姓名</span>:&nbsp;<span>' + listPageObj.ISDNAME + '</span></p><p><span class=cardIdno4>身份证</span>:&nbsp;<span>' + listPageObj.ISDIDNO + '</span></p></div></ul></li>'
                            $('.secList .mui-table-view').append(cardPageHtml2)
                            console.log('cardinfo41')

                            $('#xtdPerNum').val($('ul').find('.cardinfo4').length)
                        }

                        $('.cardinfo4').off('click')
                            // 修改card方法
                        $('.cardinfo4').on('click', function() {
                            // var cardIndex = $(this).index()
                            var cardIndex = $(this).index('ul .cardinfo4')
                            var cardJson = JSON.parse($('.listPage4HiddenInput').eq(cardIndex).val())
                            $.each(modelColumn, function(k, v) {
                                    var groupColumn = v.groupColumn
                                    $.each(groupColumn, function(k, v) {
                                        setTextValue(cardJson, v.keyName, v.type)
                                ...
                                // 修改card状态
                            cardStatus4 = 'edit'
                            editCardNo4 = cardIndex

                            // 修改上一页的地址
                            $('#cardPage4').attr('last-page', 'page1')
                                // $("#listPage4").attr("last-page", "cardPage4")
                            $('#listPage4').attr('last-page', 'page1')

                            // 4.跳转到前一页
                            // changePage("cardPage4", "listPage4", "next")
                            changePage('page1', 'listPage4', 'next')
                            console.log(cardJson)
                        })
                        ...
                        $('.deleteCardBtn').off('click')
                            // 删除card方法
                        $('.deleteCardBtn').on('click', function() {
                            var cardIndex = $(this).parents('.cardinfo4').index('ul .cardinfo4')

                            mui.confirm('是否删除该被保人？', '确认', ['是', '否'], function(e) {
                                if (e.index == 0) {
                                    $('.cardinfo4').eq(cardIndex).remove()
                                    $('.listPage4HiddenInput').eq(cardIndex).remove()
                                    $('#xtdPerNum').val($('ul').find('.cardinfo4').length)
                                } else {}
                            })
                        ...
                        if (validPage('listPage4')) {
                            ...
                            // 1.修改input的值
                            $('.listPage4HiddenInput').eq(editCardNo4).val(JSON.stringify(listPageObj))

                            // 2. 修改card页面对应card的值
                            $('.cardName4').eq(editCardNo4).html(listPageObj.BNFNAME)
                                /*$(".cardRelationship4").eq(editCardNo).html(listPageObj.BNFISDRLT_TEXT)
                                $(".cardXuhao4").eq(editCardNo).html(listPageObj.BNFORDER_TEXT)
                                $(".cardFene4").eq(editCardNo).html(listPageObj.BNFSHAREPCENT);*/
                            $('.cardIdno4').eq(editCardNo4).html(listPageObj.BNFIDNO)
                        }
                    }

                    // 添加card方法
                    $('#card4AddBtn').unbind('click')
                    $('#card4AddBtn').click(function() {
                        // 1.先将前一个页面填写的内容清空
                        $.each(modelColumn, function(k, v) {
                            var groupColumn = v.groupColumn
                            $.each(groupColumn, function(k, v) {
                    ...
                    // 添加card保持方法
                    $('#card4SaveBtn').unbind('click')
                    $('#card4SaveBtn').click(function() {
                        // 1.先将前一个页面填写的内容清空
                        $.each(modelColumn, function(k, v) {
                            var groupColumn = v.groupColumn
                            $.each(groupColumn, function(k, v) {
                        ...
                        var t = $('#cardPage4').find('.section').length
                        ...
                        cardStatus4 = 'add'
                            // 2.跳转到前一页
                        changePage('cardPage4', 'page1', 'last')
                    })
                })
                ...
                // $("#" + v.xmlNode).on("click", function() {
                //     //1.判断是否已经存在card,存在则跳转到card页，不存在则跳转到填写页
                //     if ($(".listPage4HiddenInput").length == 0) {
                //         //修改card状态
                //         cardStatus4 = "add"
                //         changePage("page1", "listPage4", "next")
                ...
                //     } else {
                //         //修改card页面
                //         $("#cardPage4 .buttonBlock").siblings(".cardinfo4").remove()
                //         for (var i = 0; i < $(".listPage4HiddenInput").length; i++) {
                //             var listPageObj = JSON.parse($(".listPage4HiddenInput").eq(i).val())
                //             //生成card页面基本信息
                //             var cardHtml = '<div class="row-group section cardinfo4"><div class="card-row4">' +
                //                 '<img src="../images/product/card-icon.png" class="card4-icon">' +
                //                 '<label class="cardName4">' + listPageObj.ISDNAME + '</label>' +
                ...
                //                 var cardIndex = $(this).parents(".cardinfo4").index()

                //                 $(".cardinfo4").eq(cardIndex).remove()
                //                 $(".listPage4HiddenInput").eq(cardIndex).remove()
                //             })
                //         }
                //         changePage("page1", "cardPage4", "next")
                //     }

                // })

                $('#xtdBtn').on('click', function() {
                    // 1.判断是否已经存在card,存在则跳转到card页，不存在则跳转到填写页
                    // if ($(".listPage4HiddenInput").length == 0) {
                    // 修改card状态
                    ...
                    //     $("#cardPage4 .buttonBlock").before(cardHtml)
                    //     console.log("cardinfo4yuan")

                    //     //生成首页列表
                    //     var cardPageHtml2 = '<li class="mui-card linum section cardinfo4 "><ul style="padding-left:0"><li><a id=icon-close class=liIcon><span class="mui-icon mui-icon-close deleteCardBtn"></span></a><span class=liNum>' + (i + 1) + '</span><div class=comtent><p><span class=cardName4>姓名</span>:&nbsp;<span>' + listPageObj.ISDNAME + '</span></p><p><span class=cardIdno4>身份证</span>:&nbsp;<span>' + listPageObj.ISDIDNO + '</span></p></div></ul></li>'
                    //     $(".secList .mui-table-view").append(cardPageHtml2)
...
function validNull(pageName) {
    ...
    } else if (pageName == 'listPage4') {
        listPage1flag = true
        $.each(productJson, function(k, v) {
            if (v.modelType == 'listPage4') {
                var modelColumn = v.modelColumn
                $.each(modelColumn, function(k, v) {
                    var groupColumn = v.groupColumn
                    $.each(groupColumn, function(k, v) {
...
function validTest(dom, fun, msg) {
    $('#pt-main').on('keyup change', dom, function() {
        var grp_val = $(dom).val()
        CarNumflag = fun($(dom).val())
        var prdId = getUrlParam('prdId')
        if (!CarNumflag) {
            $(dom).parent().next('.tip').css('display', 'block').text(msg)
        } else {
            $(dom).parent().next('.tip').css('display', 'none')
        }
    })
}
