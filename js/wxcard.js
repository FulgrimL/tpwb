...
function goBackPage() {
    ...
    } else if ($curPage.html() == "page2" || $curPage.html() == "listPage1" || $curPage.html() == "listPage2" || $curPage.html() == "listPage3" || $curPage.html() == "proClaPage") {
        check_back_status();
        setTimeout(function() {
                setWebviewTitle("我要投保");
            }, 100)
...
function setPhoneCustomer(arr, index) {
    ...
    if (isGroup == "9" || isGroup == "13") {
        if (judgeType("#GRPNAME")) {
            ...
            $("#ONTACTPSNIDNO").val(idNo).trigger("blur");
            $("#PSNBIRTH").val(birthDate);
            $('input:radio[name=PSNGENDER][value="' + sex + '"]').prop("checked", true);
            $("#PSNTELEPHONE").val(telNo).trigger("blur");
            $("#PSNEMAILADDRESS").val(email).trigger("blur");
        }
    } else {
        if (index == 0) {
            if (judgeType("#CONTACTPSN")) {
                $("#CONTACTPSN").val(name).trigger("blur");
                $("#ONTACTPSNIDNO").val(idNo).trigger("blur");
                $("#PSNBIRTH").val(birthDate);
                $('input:radio[name=PSNGENDER][value="' + sex + '"]').prop("checked", true);
                $("#PSNTELEPHONE").val(telNo).trigger("blur");
                ...
                $("#ISDIDNO").val(idNo).trigger("blur");
                $("#ISDBIRTH").val(birthDate);
                $('input:radio[name=ISDGENDER][value="' + sex + '"]').prop("checked", true);
                $("#ISDMOBILE").val(telNo).trigger("blur");
                $("#ISDEMAIL").val(email).trigger("blur");
            }

        } else if (index == 1) {
            $("#APPNAME").val(name).trigger("blur");
            switch (idType) {
                case 1:
                    console.log("1");
            ...
            $("#APPIDNO").val(idNo).trigger("blur");
            $("#APPBIRTH").val(birthDate);
            $('input:radio[name=APPGENDER][value="' + sex + '"]').prop("checked", true);
            $("#APPMOBILE").val(telNo).trigger("blur");
            $("#APPEMAIL").val(email).trigger("blur");
        } else if (index == 2) {
            if (judgeType("#ISDNAME_F")) {
                console.log("222");
                $("#ISDNAME_F").val(name).trigger("blur");
                switch (idType) {
                    case 1:
                ...
                $("#ISDIDNO_F").val(idNo).trigger("blur");
                $("#ISDBIRTH_F").val(birthDate);
                $('input:radio[name=ISDGENDER_F][value="' + sex + '"]').prop("checked", true);
                $("#ISDMOBILE_F").val(telNo).trigger("blur");
                $("#ISDEMAIL_F").val(email).trigger("blur");
            } else {
                console.log("223");
                $("#BNFNAME").val(name).trigger("blur");
                switch (idType) {
                    case 1:
                        console.log("1");
                ...
                $("#BNFIDNO").val(idNo).trigger("blur");
                $("#BNFBIRTH").val(birthDate);
                $('input:radio[name=BNFGENDER][value="' + sex + '"]').prop("checked", true);
                $("#BNFMOBILE").val(telNo).trigger("blur");
                $("#BNFEMAIL").val(email).trigger("blur");
            }
...
function createIdCard($parentCotroller, textJson) {
    ...
    if (textJson.readOnly == 0 || textJson.readOnly == undefined) {
        ...
        textHtml += '<input type="text" placeholder="' + textJson.placeHolder + '" class="mui-input-clear1" id="' + textJson.keyName + '" value="' + showValue(textJson.value) + '"  onkeyup="idCardKeyup(this,\'' + mt + '\')">';
    } else if (textJson.readOnly == 1) {
        textHtml += '<input type="text" placeholder="' + textJson.placeHolder + '" id="' + textJson.keyName + '" readonly="readonly" value="' + showValue(textJson.value) + '"  onkeyup="idCardKeyup(this,\'' + mt + '\')">';
    }
    textHtml += '</div><div class="tip"></div>';
    $parentCotroller.append(textHtml);
}
...
function createDate($parentCotroller, dateJson, option) {
    ...
    dateHtml += '<input style="width:8em" id="' + dateJson.keyName + '" class="dtPicker" placeholder="' + dateJson.placeHolder + '" readonly/>';
...
//根据所给json创建页面
function createPage2(productJson, phone, productClause, picture, noticeInsurance) {
    console.log(productClause);
    var ul = '';
    for (var i = 0; i < productClause.length; i++) {
        var index = i;
        var li = '<li class="mui-table-view-cell"><span onclick="window.open(\'' + productClause[index].clauseFile + '\')"; " >' + productClause[index].clauseName + '</li>'
        ul += li;
    }
    // mui('body').on('tap', 'a', function() { document.location.href = this.href; });
    // var imgHtml = '<div class="mui-card-content" id="imgDiv1"><img style="height:46vw;width:100%" src="' + picture + '"></div>'
    var checked = '<form class="productClause mui-table-view" onclick="changePage(\'page1\',\'proClaPage\',\'next\')"><div class="mui-table-view-cell "><span class="mui-navigate-right" style="width:100%;line-height:44px;color:#2EC076;font-size:12px" id="accountBtn"><input type="checkbox" id="wxChecked" name="vehicle" value=""  style="margin-right:5px">我已认真阅读保险条款并同意投保告知及声明与授权内容</span></div></form>'

    var SourcePhone = '<div class="mui-input-group section " id="SourcePhoneDiv"><div class="mui-input-row"><label>推荐人电话</label><input type="tel" placeholder="(选填)" class="" id="SourcePhone" value="" onkeyup="idPhoneKeyup(this,\'home\')"><div class="tip" style="display: none;">手机号不合法</div></div></div>'

    var proClaPage = '<div class="pt-page" id="proClaPage" last-page="page1"><div class="pt-triggers" ><div class="mui-collapse-content" style="padding:10px"><h4>投保人声明与授权</h4>' + noticeInsurance + '</div><ul class="mui-table-view">' + ul + '</ul><div class="wxFotter" style="height:100px;width:100%;flex: 0;"><div style="border-radius:5px;color: #2AC845;background-color: #fff;float: left;width: 35vw;text-align: center;height:10vw;padding:3vw 0;box-sizing:border-box;margin:15px" id="NbtnDiv"><span id="Nbtn" style="display:block;text-decoration:none;color:#2AC845;font-size:4vw;line-height:4vw;vertical-align: middle;">不同意</span></div><div style="border-radius:5px;color: #fff;background-color: #2AC845;float: right;width: 35vw;text-align: center;height:10vw;padding:3vw 0;box-sizing:border-box;margin:15px" id="YbtnDiv"><span id="Ybtn" onclick="" style="display:block;text-decoration:none;color:#fff;font-size:4vw;line-height:4vw;vertical-align: middle;">同意接受</span></div></div></div></div>'

    $("#pt-main").on("click", "#NbtnDiv", function() {
        if ($("#wxChecked")[0].checked) {
            $("#wxChecked")[0].removeAttribute("checked");
        }
        flag4 = false
            // $("#fixView")[0].style.display = "block";
        console.log("no")
        goBackPage()

    })

    $("#pt-main").on("click", "#YbtnDiv", function() {

        var obj = document.getElementById("wxChecked");
        ...
        switch (v.modelType) {
            ...
            case "home":
                ...
                if ($("#page1Submit").length == 0) {
                    ...
                    $("#page1 .pt-triggers").append(checked);
                    if (wxcardNo == "13") {
                        $("#page1 .pt-triggers").append(SourcePhone);

                    }


                    $("#page1 .pt-triggers").append(btnHtml);
                    // $("#page1 .pt-triggers").prepend(imgHtml);

                }

                var section = '<div class="mui-input-group section"></div>';
                $("#page1 .productClause").before(section);
                var kk = k;
                ...
                $.each(v.modelColumn, function(k, v) {
                    ...
                    if (v.type == "label") {
                        ...
                        var xxx_html2 = '<div class="mui-input-row row-title expand">' +
                            ...
                            '<input style="opacity: 0;width: 30px;height: 30px;position: absolute;right: 10px;top: 5px" id="input" type="file" accept="image/*" capture="camera" onchange="getImage(this)">' +
                            '</div>';
                    ...
                    $(".shareS").click(function() {
                        ...
                        setTimeout(function() {
                            $.ajax({
                                ...
                                // beforeSend: function() {
                                //     mask.show(); //显示遮罩
                                //     $(".mui-backdrop").append($("<div class='wxMask'><p>正在分析那中，请等待...</p><div class='loadImg'><img src='../images/pay/loading.gif' alt='' /></div>"))
            ...
            case "nextPage":
                ...
                if ($(".secList").length == 0) {
                    secListHtml = '<div class="mui-input-group section secList">' +
                        ...
                        '</li></ul></div>';
                    $("#page1 .productClause").before(secListHtml);
                } else {
